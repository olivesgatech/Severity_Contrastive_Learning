...
class SupCEResNet_OOD(nn.Module):
    """encoder + classifier"""
    def __init__(self, name='resnet50', num_classes=2):
        super(SupCEResNet_OOD, self).__init__()
    ...
    def forward(self, x):
        #return self.fc(F.normalize(self.head(self.encoder(x)),dim=1))
        return self.fc(self.encoder(x)[0]),self.encoder(x)[1]